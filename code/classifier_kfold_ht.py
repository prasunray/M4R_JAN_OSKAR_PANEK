...
for init_lr in lr_list:
   for batch_size in batch_list:
    ...
    min_val_loss = 1e12
    print("---- init_lr = %f, batch_size= %d ----" %(init_lr,batch_size))
    ...
    avg_val_loss = np.mean([metrics[0][-1] for metrics in fold_metrics])
    avg_val_accuracy = np.mean([metrics[1][-1] for metrics in fold_metrics])
